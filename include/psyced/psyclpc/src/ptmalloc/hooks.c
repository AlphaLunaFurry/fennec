...
/* $Id: hooks.c,v 1.1.1.1 2006/07/10 02:41:38 lynx Exp $ */
...
#endif
{
  ...
  _brk = MORECORE(0);
  front_misalign = (unsigned long)chunk2mem(_brk) & MALLOC_ALIGN_MASK;
  if (front_misalign > 0)
    front_misalign = MALLOC_ALIGNMENT - front_misalign;
  sbrk_size = front_misalign + mp_.top_pad + MINSIZE;
  sbrk_size += pagesz - ((unsigned long)(_brk + sbrk_size) & (pagesz - 1));
  ...
  top(&main_arena) = (mchunkptr)(_brk + front_misalign);
