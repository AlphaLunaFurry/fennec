 ...
 *
 * Make sure the call to check_state()/clear_state() is
...

{
    ...
    size_t len;
         ...
         * buffer overruns are possible. len is fippos new variable to
...
#ifdef VERBOSE
    printf("%s " PROGNAME " ready for users.\n", time_stamp());
    fflush(stdout);
#endif
    debug_message("%s " PROGNAME " ready for users.\n", time_stamp());
...
#ifdef USE_TRASH
    /* omg i can't believe i just found this completely useless line
     * in the backend loop! first i move it out, second i put it into
     * the trash. jeeesus. doesn't anybody read the code??	--lynX 2007
     */
    mud_is_up = MY_TRUE;
#endif
	...
	/* interestingly on linux this doesn't change anything
	 * you can comment it out and everything still works   *scratch*
	 */
        check_alarm();
	...
	len = 0;
        ...
        if (time_to_call_heart_beat)
        {
            ...
            time_to_call_heart_beat = MY_FALSE;
#if 1
	    /* skipping this means stopping the heartbeat..
	     * the driver will keep on working, but all
	     * callouts etc will be delayed...
	     */
            alarm(alarm_time);
#endif
...
#else
        if (!(sp->u.number & DMSG_TARGET)
	  || (sp->u.number & DMSG_STDOUT)
...
#else
	    /* when debug log is turned off, there is no
	       difference between STDOUT and LOGFILE
	       so we make sure we don't output things twice
		 --lynX */
	    if (!(sp->u.number & DMSG_TARGET)
	      || (sp->u.number & DMSG_STDOUT)
	      || (sp->u.number & DMSG_LOGFILE))
