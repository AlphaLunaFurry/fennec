...

{
    ...
    do {
        ...
        *p = -10;
         ...
         */
        printf("%s " PROGNAME " " DRIVER_VERSION LOCAL_LEVEL
               " (" PROJ_VERSION ")%s\n"
        ...
        if (!new_mudlib && chdir(MUD_LIB) == -1) {
            printf("%s Bad library directory: %s\n", time_stamp(), MUD_LIB);
            rc = 1;
            break;
        }
	...
	/* Ugly how the same sprintf is already being executed for stdout */
        debug_message("%s " PROGNAME " " DRIVER_VERSION LOCAL_LEVEL
                      " (" PROJ_VERSION ")%s\n"
                     , time_stamp(), drivertag()
                     );
        ...
        rc = exit_code;
        printf("%s " PROGNAME " shutting down.\n", time_stamp());
...
/* This option _should_ go completely, but in order to not break existing
 * scripts immediately, we just pretend it is still there and ignore what
 * it says.
 */
#if 1 /* def USE_DEBUG_LOG */
...

{
  ...
  fputs(LOCAL_LEVEL " - a multi-protocol network server application language.\n"
...

{
  ...
  version();
  fputs("\n"
"Usage: " PROGNAME " [options] [<portnumber>...]\n"
"\nOptions are:\n"
"\n"
       , stdout);
...

{
    ...
    case cMudlib:
        if (chdir(pValue) == -1) {
            fprintf(stderr, "Bad library directory: %s\n", pValue);
            return hrError;
        }
        new_mudlib = 1;
    ...
    default:
        ...
        fprintf(stderr, "%s " PROGNAME ": (eval_arg) Internal error, eOption is %d\n"
                      , time_stamp(), eOption);
        return hrError;
    } /* switch */
...

{
    ...
    for (pSrc = *ppInput; pSrc && pSrc->name != NULL; pSrc = pSrc->next)
    {
        ...
        {
            fprintf(stderr
                   , PROGNAME ": Recursion in nested argument files: %s\n"
                   , pName);
            for (pSrc = *ppInput; pSrc && pSrc->name != NULL; pSrc = pSrc->next)
    ...
    if (f == NULL)
    {
        int err = errno;
        fprintf( stderr
               , PROGNAME ": Can't open argument file '%s' for reading: (%d) %s\n"
    ...
    if (fstat(fileno(f), &st) == -1)
    {
        int err = errno;
        fprintf( stderr
               , PROGNAME ": Can't stat argument file '%s': (%d) %s\n"
               , pName, err, strerror(err)
               );
        fclose(f);
        return MY_FALSE;
    ...
    /* Get a new input source structure and read in the file. */
    {
        ...
        {
            fprintf( stderr
                   , PROGNAME ": Out of memory reading argument file '%s'\n"
                   , pName
                   );
            fclose(f);
            return MY_FALSE;
        ...
        if (1 != fread(pData, size, 1, f))
        {
            int err = errno;
            fprintf( stderr
                   , PROGNAME ": Error reading argument file '%s': (%d) %s\n"
                   , pName, err, strerror(err)
                   );
            free(pSrc);
            fclose(f);
    ...
    for (left = 0; left < size && *pData != '\0'; left++, pData++)
    {
            ...
            )
        {
            ...
            if (c == '\r' || c == '\n')
            {
                ...
                {
                    fprintf( stderr
                           , PROGNAME ": Error in argument file '%s': "
    ...
    {
        fprintf( stderr
               , PROGNAME ": Warning: Argument file '%s' contained no data.\n"
               , pName);
  ...
  while (pInput != NULL)
  {
      ...
      for (iArg = pInput->arg; iArg < pInput->argc; !bCont ? iArg++ : iArg)
      {
        ...
        if (cUnknown == eOption)
        {
          ...
          {
            if (pInput->name != NULL)
                fprintf(stderr, PROGNAME ": (%s) Unknown option '", pInput->name);
            else
                fputs(PROGNAME ": Unknown option '", stderr);
            if (bShort)
