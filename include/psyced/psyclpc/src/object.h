...
#endif
#if 1 /* what are these so important for? */
...
#endif
#ifdef USE_SQLITE
    /* should probably be implemented as a 'sent' extension (see above) */
    Bool open_sqlite_db;   /* does this object have an open sqlite db? */
#endif 
#ifdef USE_EXPAT
    /* should probably be implemented as a 'sent' extension (see above) */
    Bool xml_error;
    XML_Parser xml_parser; /* does this object have an expat xml parser? */
#endif
    svalue_t *variables;
      /* All variables to this object: an array of svalues, allocated
       * in a separate block.
       */
#if 1 /* USE_EVALUATION_COST */
