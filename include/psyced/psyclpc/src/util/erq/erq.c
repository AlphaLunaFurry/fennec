 ...
 * in the form "YYYY.MM.DD HH:MM:SS [erq]".
    ...
    do {
        ...
        if (num <= 0)
        {
            if (!num)
            {
                ...
                return 0;
#else
		/* Driver has terminated. Let's go home, too   --lynX */
		exit(0);
#endif
            }
            else
            {
#if ERQ_DEBUG > 0
                sprintf(buf, "%s process %d exiting", time_stamp(), getpid());
		perror(buf);
#endif
		/* Driver has terminated. Let's go home, too   --lynX */
		exit(1);
...

{
    ...
    for (;;) /* The Loop (tm) */
    {
        ...
        {
            if (num < 0) {
		sprintf(buf, "%s %d read", time_stamp(), getpid());
		perror(buf);
		exit(1);	/* should i even try to continue? */
            } else
...
#else
		fprintf(stderr, "%s: ERQ_LOOKUP_SRV requested but not compiled into erq binary!\n", time_stamp());
#endif
		break;
	      }
...

{
    ...
    /* Quick and dirty commandline parser */
    {
        ...
        {
            fprintf(stderr, "%s dynamic attachment not implemented.\n", time_stamp());
            goto die;
        }
    }
